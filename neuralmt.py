...
class AttentionModule(nn.Module):
    ...
    def forward(self, decoder_hidden, encoder_out):
        ...
        # ----- OUR CODE: Attention Fixes -----
        alphaVectorSum = torch.sum(alpha*encoder_out.permute(1, 0, 2), dim=1)
        context = (alphaVectorSum).reshape(1, 1, dim)
        # ----- END HERE -----
...
def translate(model, test_iter):
    ...
    for i, batch in tqdm(enumerate(test_iter)):
        ...
        # ----- OUR CODE: Unkown Replacement, Duplicated Removal -----
        # Unkown Word Replacement
        output = output.replace('<unk>', 'the')

        # Removing 2x word in a row
        res = []
        temp = output.split()
        ...
        # # Removing 3x word in a row
        # res = []
        # temp = output.split()
        # for i in range(len(temp)):
        #     if i > 1 and temp[i] == temp[i-1] == temp[i-2]:
        #         continue
        #     else:
        #         res.append(temp[i])
        # out = " ".join(res)
...
if __name__ == '__main__':
    ...
    optparser.add_option(
        "-m", "--model", dest="model", default=os.path.join('data', 'seq2seq_E047.pt'), 
        help="model file")
    optparser.add_option(
